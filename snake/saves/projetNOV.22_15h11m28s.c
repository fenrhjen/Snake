...
void printScore(int* pointeurScore){
  char str[15];
  sprintf(str, "%05d", *pointeurScore);
  EcrireTexte(60*TAILLE_CASE+BORD-TailleChaineEcran("00000",2),40*TAILLE_CASE+BORD+30,str,2);
}

void printScreen(int snake[TAILLE_SNAKE][2], int len_snake, int pommes[TAILLE_POMME][2], int len_pommes, unsigned long int time, int* pointeurScore){
  EffacerEcran(CouleurParNom("black"));
  printTerrain(snake, len_snake, pommes, len_pommes);
  ChoisirCouleurDessin(CouleurParNom("white"));
  printScore(pointeurScore);
  printTime(time);
...
void initSnake(int snake[TAILLE_SNAKE][2],int len_snake){
  int i;
  /*ON INITIALISE TOUTES LES CASES DU SNAKE (POUR ENSUITE EN AFFICHER QUE LEN_SNAKE CASES)*/
...
void initPommes(int pommes[TAILLE_POMME][2], int len_pommes,int snake[TAILLE_SNAKE][2]){
  int i;
  for (i=0; i<len_pommes; i++){
    if(pommes[i][0]!=POS_X_HEAD_SNAKE && pommes[i][1]!=POS_Y_HEAD_SNAKE){/*pour ne pas initialiser une pomme sur la pos du snake de départ*/
   placePomme(pommes, i);
    }
}
}
void ActualiserPosPomme(int pommes[TAILLE_POMME][2], int len_pommes,int snake[TAILLE_SNAKE][2]){
  int i;
  for(i=0;i<len_pommes;i++){
    if(pommes[i][0]==POS_X_HEAD_SNAKE && pommes[i][1]==POS_Y_HEAD_SNAKE){
      placePomme(pommes,i);
    }
  }
}
...
void CheckCollisionPomme(int len_pommes,int pommes[TAILLE_POMME][2],int snake[TAILLE_SNAKE][2],int* pointeurLenSnake, int* pointeurScore){
  /********CHECK POMME MIAMIAM*******/
  int i;
  /*AUGMENTATION SCORE*/
  for(i=0;i<len_pommes;i++){
    if(POS_X_HEAD_SNAKE==pommes[i][0] && POS_Y_HEAD_SNAKE==pommes[i][1]){
      *pointeurScore+=5;
      /*AUGMENTATION TAILLE SNAKE*/
      *pointeurLenSnake+=1;/*AUGMENTATION LEN_SNAKE DE 1*/
    }
...
int main(void){
  ...
  int snake[TAILLE_SNAKE][2],
      ...
      direction[2]={0,-1};
  int score=0;
  int *pointeurScore= &score, *pointeurLenSnake=&len_snake;
  ...
  initPommes(pommes,len_pommes,snake);
  ...
  */
  while(1){
    if(Microsecondes()>prochainAffichage){
      ActualiserPosPomme(pommes,len_pommes,snake);
      printScreen(snake, len_snake,  pommes, len_pommes, tempsDepart, pointeurScore);
    ...
    if(Microsecondes()>prochainDeplacement){
      ...
      CheckCollisionPomme(len_pommes,pommes,snake,pointeurLenSnake,pointeurScore);
