...
void CheckCollisionPomme(int len_pommes,int pommes[TAILLE_POMME][2],int snake[TAILLE_SNAKE][2], int* pointeurScore){
  /********CHECK POMME MIAMIAM*******/
  int i;
  for(i=0;i<len_pommes;i++){
  ...
  */
  while(1){
    ...
    if(Microsecondes()>prochainDeplacement){
      ...
      CheckCollisionPomme(len_pommes,pommes,snake,pointeurScore);
